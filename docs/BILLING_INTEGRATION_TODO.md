...
| 플랜 | 가격 | AI 통화요약 | 비고 |
|------|------|-------------|------|
| Free | 0원 | ✗ (체험 5회 별도) | 고객관리대장, 양식 커스터마이징, 문자발송, 마케팅 분석, AI 사주/로또, 개인 업로드 1KB |
| Plus | 19,000원/월 | 월 20회 | Free 전체 + AI 통화요약 + 원터치 저장 |
| Pro  | 39,000원/월 | 무제한 | 모든 기능 |

**사용량 카운트 단위**: 통화녹음 → AI 요약 → 고객관리대장 저장 = **1회**.
요약과 저장을 따로 차감 X.

**결제 방식**: 월 정기구독 (카드 빌링키). 매월 자동 초기화.

**결제수단**: 신용카드, 체크카드, 토스페이, 카카오페이, (네이버페이 추후)

**환불 정책**:
- 결제 후 사용 이력 없으면 전액 환불
- 사용 이력 있으면 부분 환불 제한 가능
- 중도 해지 시 다음 결제일부터 자동결제 중단 — 남은 기간 사용 가능
...
## D. 기능 게이팅 UI (가격 확정 기반 카피)

- [ ] **D1.** 통화 종료 → AI 요약 자동 업로드 전 사용량 사전 검증
      - `GET /api/billing/status` 또는 `/api/usage/check-summary` 호출
      - `allow=false` 면 업로드 skip + 안내 모달
- [ ] **D2.** 안내 모달 카피 (가격 확정)
      - free + 체험 0회: "체험 5회를 모두 사용했어요. Plus(월 19,000원, 월
        20회) 또는 Pro(월 39,000원, 무제한) 구독을 시작해보세요."
      - plus 20회 소진: "이번 달 Plus 20회를 모두 사용했어요. Pro(무제한)로
        업그레이드하거나 다음 결제일까지 기다려주세요."
      - trialing 1~4회 남음: 무료 체험 카운트다운 표시 (모달 X, 인디케이터만)
- [ ] **D3.** 메인/설정 화면에 사용량 인디케이터:
      - Free: "체험 X회 남음"
      - Plus: "이번 달 AI 요약: 7/20회"
      - Pro: "AI 요약 무제한" (또는 인디케이터 숨김)
...
- [ ] **F2.** 해지 후 잔여 기간 사용 — `plan_status === 'cancelled'` 이지만
      `current_period_end` 이전이면 기능 정상 동작. UI는 "다음 결제 없음 —
      MM-DD까지 사용 가능" 안내.

## H. 필수 정책 페이지 (한국 PG 심사 통과 + 사용자 보호 의무)

웹 측에서 페이지를 호스팅하고 앱은 설정 메뉴에서 진입점만 제공:

- [ ] **H1.** 이용약관 (`/legal/terms`)
- [ ] **H2.** 개인정보처리방침 (`/legal/privacy`)
- [ ] **H3.** 환불정책 (`/legal/refund`)
- [ ] **H4.** 자동결제 안내 (`/legal/recurring`)
- [ ] **H5.** 사업자정보 표시 (어센트라 / 사업자등록번호 / 대표자 / 주소 /
      연락처) — Play Store 앱 상세 페이지에도 표기
- [ ] **H6.** 설정 화면에 위 5개 페이지 진입 row (WebView로 표시)

## I. 보안 / 데이터

확정된 정책에 맞춰 클라이언트 측 자기검열:
- [ ] **I1.** Secret Key / Billing Key를 클라이언트에 절대 노출 X (전부 서버
      ENV — 이미 PortOne API 직접 호출 안 하므로 자동 만족)
- [ ] **I2.** 민감 데이터 (요약 내용, 고객 정보) → 서버에서 AES-256 암호화
      저장. 앱은 평문으로 받아서 표시. 캐시 저장 시 OS 기본 sandbox에 의존
      (별도 디스크 암호화 X — Android 12+ 기본 디스크 암호화로 충분)
- [ ] **I3.** Logcat 출력에 JWT / 결제 정보 노출 안 되는지 audit
      (`__DEV__` 가드 + `accessToken.length` 만 찍는 패턴 유지)
